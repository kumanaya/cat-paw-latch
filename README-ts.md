...
apps/
  desktop/       Electron app: device-core in the main process; tray, approval windows,
                 and the post-onboarding UI — a Liquid Glass sidebar beside a solid
                 content workspace. Its design tokens live in the header of
                 apps/desktop/src/renderer/styles.css, and that file is the only
                 source of truth for the app's look.
fixtures/        golden conformance vectors, GENERATED BY the Swift code (see below)
```

**Two design systems, on purpose, and the line between them is a file list.**
Setup (`onboarding.html`) keeps its own identity: it loads `onboarding.css` and
`switch.css` and **nothing** from `styles.css`, so no rule in the app's system
can reach the wizard — that is the whole of the guarantee, and it is why
`onboarding.css` is not refactored alongside `styles.css`. `switch.css` is the
one file both windows load; it is driven entirely by `--switch-*` tokens, so
each host maps its own palette onto it and neither has to change. `vault.css`
and `fdapanel.css` are scoped to their own surfaces (`.vaultui`, the grant
panel) and read the same tokens.
