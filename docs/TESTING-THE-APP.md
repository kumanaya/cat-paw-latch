...
| Automation for Messages **granted**, Ask mode; snapshot with the `imessage` skill's `verifySendSnapshot` first | the skill's participant recipe: `plow_run_applescript` `app: "Messages"`, `args: ["<text>", "<your phone or email>"]` | the card shows "Script Messages (com.apple.MobileSMS)", the script, and both values as `item 1 of argv` / `item 2 of argv`; approved, `completed`, exit 0; `verifySend` (with that handle) returns one row newer than the snapshot with `is_sent` 1, then `is_delivered` 1 |
| Same, to a group | the skill's chat recipe, `args: ["<text>", "<guid from recentChats>"]` | as above; `verifySend` scoped by that chat guid returns the new row, `is_sent` 1 / `is_delivered` 1 |
| Same, with a file | the skill's attachment recipe, `args: ["<absolute path to a small image>", "<your phone or email>"]` | as above, and the image arrives on the other device; `verifySend` returns the new row with `is_sent` 1 / `is_delivered` 1 |
