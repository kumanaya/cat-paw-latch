...
| Same, to a group | the skill's chat recipe, `args: ["<text>", "<guid from recentChats>"]` | as above; `verifySend` scoped by that chat guid returns the new row, `is_sent` 1 / `error` 0 |
