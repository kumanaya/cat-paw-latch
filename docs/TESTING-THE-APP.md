...
| Mail compose (`make new outgoing message`) with Automation **granted** | `plow_run_command` `osascript` with `apple_events: true` | `app_refuses_sandboxed_sender` / `confirmed` / no permission, `retry: with_plow_run_applescript`: Mail refuses the command from any sandboxed sender (bare `osascript` works, an allow-everything seatbelt gets `-10004`); the Audit row says so and nothing lands on the Permissions section, since no switch changes it |
...
| Automation for Messages **granted**, Ask mode; snapshot with the `imessage` skill's `verifySendSnapshot` first | the skill's participant recipe: `plow_run_applescript` `app: "Messages"`, `args: ["<text>", "<your phone or email>"]` | the card shows "Script Messages (com.apple.MobileSMS)", the script, and both values as `item 1 of argv` / `item 2 of argv`; approved, `completed`, exit 0; `verifySend` (with that handle) returns one row newer than the snapshot with `is_sent` 1 and `error` 0 (`is_delivered` may stay 0 — a receipt need never come back) |
| Same, to a group | the skill's chat recipe, `args: ["<text>", "<guid from plow-messages chats>"]` | as above; `verifySend` scoped by that chat guid returns the new row, `is_sent` 1 / `error` 0 |
| Same, with a file | the skill's attachment recipe, `args: ["<absolute path to a small image>", "<your phone or email>"]` | as above, and the image arrives on the other device — that arrival is the end-to-end check here; `verifySend` returns the new row with `is_sent` 1 / `error` 0 |
| Automation for Messages **denied** | `plow_run_applescript` `app: "Messages"`, `tell application "Messages" to get name` | `macos_permission` / `confirmed` / `automation`, `probes.ran_sandboxed: false`; the Permissions section's Automation row for Messages gains the block |
...
| Owner-facing | any confirmed block above | the tray gains a "Needs …" item; a notification carries the owner sentence; both land on Settings' Permissions section when the block names a switch, and on the Audit tab's Blocked view when it names none at all (a locked file, SIP, POSIX permissions); the item comes down on its own when the owner answers the dialog and the run goes on; the Audit row's Status reads "Blocked · <permission>" in amber beside a green Allowed decision, and the Status filter's `Blocked` shows it |
