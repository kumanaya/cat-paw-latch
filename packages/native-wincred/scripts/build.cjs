...
// The Credential Manager provider is one of several the key store can use, so
// a machine that cannot compile it (no VS Build Tools, CI on macOS/Linux)
// must still be able to `just install` — the store falls back to safeStorage
// or the key file. A real build failure is printed, not hidden.
//
...
if (process.platform !== "win32") {
  console.log("@domo/native-wincred: not Windows, skipping build");
  process.exit(0);
}
...
if (status !== 0) {
  console.warn(
    "@domo/native-wincred: build failed (see above). " +
      "The vault falls back to safeStorage/file for its master key; " +
      "install the VS Build Tools and `npm rebuild @domo/native-wincred` to enable Credential Manager.",
  );
}
process.exit(0);
