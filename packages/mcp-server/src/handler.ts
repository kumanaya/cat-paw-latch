...
A call can also return status 'denied'. Relay and obey its reason: different denial sources have different recovery. When Gatekeeper's AI Reviewer denied the request, always tell the user to open Plow Latch on their Mac, where they can review the denial or improve their Gatekeeper instructions. For any other 'denied' result, follow its reason rather than inventing a wait or retry.
...
A call can also return status 'denied'. Relay and obey its reason: different denial sources have different recovery. When Gatekeeper's AI Reviewer denied the request, always tell the user to open Plow Latch on their PC, where they can review the denial or improve their Gatekeeper instructions. For any other 'denied' result, follow its reason rather than inventing a wait or retry.
