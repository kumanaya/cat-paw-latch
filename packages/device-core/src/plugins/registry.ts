...
export function loadPlugins(roots: readonly string[]): StagedPlugin[] {
  ...
  const platform = process.platform as PluginPlatform;
  ...
  for (const root of roots) {
    ...
    for (const name of names) {
      ...
      // Windows looks for `<name>.exe`, the name `stageBinaries` writes there.
      const stagedForPlatform = (b: PluginManifest["runtime"]["binaries"][number]): boolean =>
        (platform === "darwin" || b.platforms[platform] !== undefined) &&
        executable(path.join(bin, stagedName(b.name, platform)));
      if (!manifest.runtime.binaries.every(stagedForPlatform)) continue;
