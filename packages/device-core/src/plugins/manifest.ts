...
/**
 * The OS axis of a plugin's staged binaries. `darwin` is expressed by the
 * top-level `url`/`sha256`/`executable` (the shape upstream's manifests already
 * have); every other platform the plugin supports is a block under
 * `platforms`, so a plugin that is macOS-only stays exactly as it was.
 */
export type PluginPlatform = "darwin" | "win32" | "linux";

export interface PlatformBinary {
  url: Record<Arch, string>;
  sha256: Record<Arch, string>;
  executable?: string;
}
...
export interface PluginManifest {
  ...
  runtime: {
    binaries: {
      name: string;
      /** The darwin build: the default, and the only platform upstream ships. */
      url: Record<Arch, string>;
      sha256: Record<Arch, string>;
      executable?: string;
...
export function parseManifest(raw: string): PluginManifest {
  ...
  const binaries = typedArray(runtime.binaries, "runtime.binaries").map((b: unknown) => {
    ...
    const spec = (raw: unknown, what: string): PlatformBinary => {
      const s = obj(raw);
      const url = obj(s.url);
      const sha256 = obj(s.sha256);
      for (const arch of ARCHES) {
        if (typeof url[arch] !== "string" || !(url[arch] as string).startsWith("https://")) {
          fail(`binary ${bname} needs an https url for ${what} arm64 and x64`);
        }
        if (typeof sha256[arch] !== "string" || !SHA.test(sha256[arch] as string)) {
          fail(`binary ${bname} needs a sha256 for ${what} arm64 and x64`);
        }
      }
      return {
        url: { arm64: url.arm64 as string, x64: url.x64 as string },
        sha256: { arm64: sha256.arm64 as string, x64: sha256.x64 as string },
        ...(s.executable === undefined
          ? {}
          : { executable: insideOrFail(s.executable, `binary ${bname} executable`) }),
      };
    };
    // The top-level block IS darwin's — upstream's shape, and the only shape a
    // macOS-only plugin needs. A platform block under `platforms` carries that
    // OS's own pins; darwin may not be redeclared there (one spelling each).
    const platforms: Partial<Record<PluginPlatform, PlatformBinary>> = {};
    for (const [platform, raw] of Object.entries(typedObj(bin.platforms, `binary ${bname} platforms`))) {
      if (!(PLATFORMS as readonly string[]).includes(platform)) {
        fail(`binary ${bname} platforms must be darwin, win32 or linux`);
      }
      if (platform === "darwin") fail(`binary ${bname} must not redeclare darwin; the top-level url and sha256 are darwin's`);
      platforms[platform as PluginPlatform] = spec(raw, platform);
    }
    return {
      name: bname,
      ...spec({ url: bin.url, sha256: bin.sha256, executable: bin.executable }, "darwin"),
      platforms,
    };
  });
