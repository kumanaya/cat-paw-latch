/**
 * The audit log as one row per operation — the grouping and the classification
 * the desktop's Audit tab shows, and what `plow_history` hands an agent.
 * Pure: no Electron, no DOM. One copy, because a reader that re-derives which
 * event sequence means "completed", "denied" or "blocked" drifts from this
 * table the day either changes (see #336's review history for the three
 * rounds that proved it). The desktop's live index (apps/desktop's
 * auditIndex.ts) folds events through the same ActivityGrouper.
 */
...
function deniedOperation(ev: ReturnType<typeof jv>): [string, BadgeTone, StatusKind] {
  switch (ev.get("cause").str ?? "outside_approved_bound") {
    ...
    // The command may be over while a job it left running still holds the
    // path; the words must not send the owner looking for a finished process.
    case "busy": return ["Failed · path in use by a command or background job", "amber", "failed"];
