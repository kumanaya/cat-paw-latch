...
function intent(argv: string[]): Intent {
  return makeIntent({
    agentId: "a",
    agentDisplay: "a",
    deviceId: "d",
...
function truncate(i: Intent): Intent {
  return {
    ...i,
    capabilities: i.capabilities.map((c) =>
      c.kind === "process.exec" ? { ...c, argv: c.argv!.slice(0, 2) } : c,
    ),
  };
}

describe("PolicyEngine ruleView", () => {
  // A stored rule for a `process.exec` intent is refused outright on Windows
  // and Linux (no presence gate on those hosts), so the view can only be
  // observed where standing exec rules exist: macOS.
  it.skipIf(process.platform !== "darwin")(
    "stores a rule under the viewed capabilities and answers a sibling from it", async () => {
    ...
    expect(engine.allRules()[0].capabilities.find((c) => c.kind === "process.exec")?.argv).toEqual([
      "fix",
      "query",
    ]);
    ...
    expect(grant.capabilities.find((c) => c.kind === "process.exec")?.argv).toEqual([
      "fix",
      "query",
      "second",
    ]);
