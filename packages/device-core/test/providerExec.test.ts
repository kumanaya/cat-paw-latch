...
describe("a provider through the exec path", () => {
  ...
  itSpawns("leaves a non-provider command completely alone", async () => {
    const mint = vi.fn(async () => TOKEN);
    const plugins = gogPlugin();
    const plugin = plugins[0]!;
    // Linux will not exec a live `/bin/echo` — argv[0] must sit under an
    // approved root that the run's workspace stages. Put a copy beside the
    // stand-in gog and approve that directory, so the command is ordinary
    // exec, not the provider path.
    const echo =
      process.platform === "linux"
        ? (() => {
            const bin = path.join(plugin.binDir, "echo");
            fs.copyFileSync("/usr/bin/echo", bin);
            fs.chmodSync(bin, 0o755);
            return bin;
          })()
        : "/bin/echo";
    const d = device(minterOf(mint), plugins);
    const out = String(jv(await run(d, [echo, "hello"], 8000, [plugin.binDir])).get("output").str ?? "");
...
describe("a staged non-provider plugin through the exec path", () => {
  ...
  itSpawns(
    ...
    async () => {
      ...
      // The Linux cage stages approved roots into its own workspace, so there
      // the child's pwd is the mapped copy under bwrap-workspace; still the
      // plugin's directory that was staged, never the bare scratch dir.
      if (process.platform === "linux") {
        expect(out).toMatch(/CWD=.*bwrap-workspace/);
      } else {
  ...
  // macOS only: the entrypoint falls through to the curated PATH, and the
  // Linux cage deliberately cannot exec a live `/usr/bin` tool from an
  // approved root's staged script (bubblewrap binds libraries, never
  // `/usr/bin`) — so the property this pins has no Linux spelling.
 ...
 *
 * macOS only, deliberately: Windows and Linux refuse a standing
 * `process.exec` rule outright (`PolicyEngine`'s `ruleEligibleCapabilities` —
 * no presence gate on those hosts), so a plugin read is decided fresh every
 * time there and the narrowed view is never consulted.
 */
describe.skipIf(process.platform !== "darwin")("a plugin's always-allow rule, narrowed by argv shape", () => {
