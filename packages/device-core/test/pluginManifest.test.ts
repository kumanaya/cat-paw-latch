...
describe("parseManifest", () => {
  ...
  it("accepts a full manifest: binaries, a source, exec in that source, a daemon and a hook", () => {
    ...
    expect(m.runtime.binaries).toEqual([{ ...bin, platforms: {} }]);
  ...
  it("accepts per-platform pins: darwin top-level, every other OS under platforms", () => {
    const m = parseManifest(withPatch({
      runtime: { binaries: [{
        name: "b",
        url: { arm64: "https://x/darwin-arm64", x64: "https://x/darwin-x64" },
        sha256: { arm64: "a".repeat(64), x64: "b".repeat(64) },
        platforms: {
          win32: {
            url: { arm64: "https://x/win-arm64", x64: "https://x/win-x64" },
            sha256: { arm64: "c".repeat(64), x64: "d".repeat(64) },
            executable: "b.exe",
          },
          linux: {
            url: { arm64: "https://x/linux-arm64", x64: "https://x/linux-x64" },
            sha256: { arm64: "e".repeat(64), x64: "f".repeat(64) },
          },
        },
      }], sources: [] },
    }));
    const b = m.runtime.binaries[0]!;
    expect(b.url.arm64).toBe("https://x/darwin-arm64");
    expect(b.platforms.win32?.executable).toBe("b.exe");
    expect(b.platforms.win32?.sha256.arm64).toBe("c".repeat(64));
    expect(b.platforms.linux?.url.x64).toBe("https://x/linux-x64");
  });
  ...
  it.each([
    ...
    ["a binary without a sha for every arch", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64) } }], sources: [] } }), "binary b needs a sha256 for darwin arm64 and x64"],
    ["a binary with a non-https url", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "http://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } }], sources: [] } }), "binary b needs an https url for darwin arm64 and x64"],
    ...
    ["a platform block for an OS the schema does not know", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) }, platforms: { plan9: { url: { arm64: "https://x/9", x64: "https://x/9" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } } } }], sources: [] } }), "binary b platforms must be darwin, win32 or linux"],
    ["a darwin block redeclaring the top-level pins", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) }, platforms: { darwin: { url: { arm64: "https://x/d", x64: "https://x/d" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } } } }], sources: [] } }), "binary b must not redeclare darwin; the top-level url and sha256 are darwin's"],
    ["a platform block missing a sha", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) }, platforms: { win32: { url: { arm64: "https://x/w", x64: "https://x/w" }, sha256: { arm64: "a".repeat(64) } } } }], sources: [] } }), "binary b needs a sha256 for win32 arm64 and x64"],
    ["a platform block with a non-https url", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) }, platforms: { linux: { url: { arm64: "http://x/l", x64: "https://x/l" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } } } }], sources: [] } }), "binary b needs an https url for linux arm64 and x64"],
    ["a platform executable that escapes the plugin", withPatch({ runtime: { binaries: [{ name: "b", url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) }, platforms: { win32: { executable: "/b.exe", url: { arm64: "https://x/w", x64: "https://x/w" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } } } }], sources: [] } }), "binary b executable must be a path inside the plugin"],
    ...
    ["a binary with a non-string name", withPatch({ runtime: { binaries: [{ name: true, url: { arm64: "https://x/b", x64: "https://x/b" }, sha256: { arm64: "a".repeat(64), x64: "a".repeat(64) } }], sources: [] } }), "binary name must be a string"],
