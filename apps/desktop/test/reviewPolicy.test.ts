...
describe("a queued dialog is answered by a rule stored ahead of it", () => {
  ...
  // Standing rules exist only on macOS: on Windows/Linux a sensitive approval
  // never persists (ruleEligibleCapabilities), so a queued twin gets its own
  // dialog — the platform tests own that behavior.
  it.skipIf(process.platform !== "darwin")("grants the twins behind an 'always allow' as the rule, with no dialog", async () => {
  ...
  it.skipIf(process.platform !== "darwin")("[A, B, A] answered 'always allow' on A leaves the human with [B]", async () => {
  ...
  it.skipIf(process.platform !== "darwin")("an 'always allow' after the approval's deadline keeps its rule — only the request expires", async () => {
