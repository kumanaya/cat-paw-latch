...
describe("applyPlowFolderIcon", () => {
  it.skipIf(process.platform !== "darwin")("passes the workspace and badge paths to the native helper", async () => {
...
`,
  ...
  it.skipIf(process.platform !== "darwin")("keeps cosmetic failure out of startup and logs no helper detail", async () => {
    const helper = executable(
      "fail.mjs",
      `#!/usr/bin/env node
