...
export function capabilitiesView(input: CapabilitiesInput): CapabilitiesView {
  ...
  // inherits it. On macOS that is `child_attribution` (`ok` vs `broken`). On
  // Windows and Linux the check is `not_applicable` — there is no TCC
  // inheritance — so a grant that is not `broken` is inherited.
  const inherited = fda === true && inv?.child_attribution.status !== "broken";
  ...
  items.push(
    row(
      ...
      // a grant a child cannot use is not access. Windows/Linux have no TCC
      // list to re-add to, so the hint stays off there.
      appleOnly
        ? (fda === true && !inherited
