...
1. In `latch-plugin.json` set `version`; the version segment and `sha256` of
   the darwin top-level pins AND of every `platforms` block — darwin tarballs,
   Windows `.zip`s and Linux tarballs, each arch, checked against upstream's
   `checksums.txt`. A stale url downloads the old release and fails the sha
   check — the pin is the sha, the url is where it comes from. The darwin
   member is `gog`; Windows stages `gog.exe` (`executable`).
